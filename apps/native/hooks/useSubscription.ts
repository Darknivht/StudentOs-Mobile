...
const FREE_LIFETIME: LifetimeLimits = {
  totalNotes: 15,
  totalQuizzes: 20,
  totalFlashcardSets: 10,
  totalAIUses: 30,
};
const PLUS_LIFETIME: LifetimeLimits = {
  totalNotes: 100,
  totalQuizzes: 100,
  totalFlashcardSets: 50,
  totalAIUses: 200,
};
const PRO_LIFETIME: LifetimeLimits = {
  totalNotes: Infinity,
  totalQuizzes: Infinity,
  totalFlashcardSets: Infinity,
  totalAIUses: Infinity,
};
...
const FULL_ACCESS: SubscriptionData = {
  ...
  jobSearchesThisMonth: 0,
  jobSearchesResetMonth: "",
  limits: PRO_LIMITS,
  lifetimeLimits: PRO_LIFETIME,
  totalNotesCount: 0,
  totalQuizzesCount: 0,
  totalFlashcardSetsCount: 0,
  totalAIUsesCount: 0,
  canUseAI: true,
  canCreateQuiz: true,
  ...
  const fetchSubscription = useCallback(async () => {
    ...
    try {
      const [
        ...
        flashcardsCountRes,
        aiCountRes,
        examAttemptsCountRes,
      ] = await Promise.all([
        supabase
          .from("profiles")
          .select(
            "subscription_tier, subscription_expires_at, ai_calls_today, ai_calls_reset_at, quizzes_today, flashcards_generated_today, notes_today, job_searches_this_month, job_searches_reset_month"
          )
          .eq("user_id", user.id)
          .single(),
        supabase
      ...
      const aiCallsToday =
        data?.ai_calls_reset_at === today ? data?.ai_calls_today || 0 : 0;
      const quizzesToday =
        data?.ai_calls_reset_at === today ? data?.quizzes_today || 0 : 0;
      const flashcardsToday =
        data?.ai_calls_reset_at === today
          ? data?.flashcards_generated_today || 0
          : 0;
      const notesToday =
        data?.ai_calls_reset_at === today ? data?.notes_today || 0 : 0;
      ...
      const jobSearchesThisMonth =
        (data as any)?.job_searches_this_month || 0;
      const jobSearchesResetMonth =
        (data as any)?.job_searches_reset_month || "";
      ...
      setSubscription({
        ...
        notesToday,
        examQuestionsToday,
        jobSearchesThisMonth,
        jobSearchesResetMonth,
        limits,
        lifetimeLimits,
        totalNotesCount,
        totalQuizzesCount,
        totalFlashcardSetsCount,
        totalAIUsesCount,
        canUseAI:
          isPro ||
          isPlus ||
          (aiCallsToday < limits.aiCallsLimit &&
            totalAIUsesCount < lifetimeLimits.totalAIUses),
        canCreateQuiz:
          isPro ||
          isPlus ||
          (quizzesToday < limits.quizzesLimit &&
            totalQuizzesCount < lifetimeLimits.totalQuizzes),
        canCreateFlashcard:
          isPro ||
          isPlus ||
          (flashcardsToday < limits.flashcardsLimit &&
            totalFlashcardSetsCount < lifetimeLimits.totalFlashcardSets),
        canCreateNote:
          isPro ||
          isPlus ||
          (notesToday < limits.notesLimit &&
            totalNotesCount < lifetimeLimits.totalNotes),
        canUseChat: true,
  ...
  const gateFeature = useCallback(
    (
      ...
      if (type === "jobSearch") {
        ...
        const usage =
          subscription.jobSearchesResetMonth === currentMonth
            ? subscription.jobSearchesThisMonth
            : 0;
      ...
      const lifetimeMap = {
        ai: {
          current: subscription.totalAIUsesCount,
          limit: subscription.lifetimeLimits.totalAIUses,
        },
        quiz: {
          current: subscription.totalQuizzesCount,
          limit: subscription.lifetimeLimits.totalQuizzes,
        },
        flashcard: {
          current: subscription.totalFlashcardSetsCount,
          limit: subscription.lifetimeLimits.totalFlashcardSets,
        },
        note: {
          current: subscription.totalNotesCount,
          limit: subscription.lifetimeLimits.totalNotes,
        },
      };

      const lifetime =
        lifetimeMap[type as keyof typeof lifetimeMap];
      if (lifetime && lifetime.current >= lifetime.limit) {
        return {
          allowed: false,
          reason: "lifetime",
          currentUsage: lifetime.current,
      ...
      const dailyMap = {
        ai: {
          current: subscription.aiCallsToday,
          limit: subscription.limits.aiCallsLimit,
        },
        quiz: {
          current: subscription.quizzesToday,
          limit: subscription.limits.quizzesLimit,
        },
        flashcard: {
          current: subscription.flashcardsToday,
          limit: subscription.limits.flashcardsLimit,
        },
        note: {
          current: subscription.notesToday,
          limit: subscription.limits.notesLimit,
        },
      };
      ...
      return {
        allowed: true,
        reason: null,
        currentUsage: daily?.current ?? 0,
        limit: daily?.limit ?? Infinity,
        isLifetime: false,
        requiredTier: "plus",
      };
    },
  ...
  const incrementUsage = async (
    ...
    if (type === "examQuestion") {
      setSubscription((prev) => ({
        ...prev,
        examQuestionsToday: prev.examQuestionsToday + 1,
      }));
      return true;
    }
    ...
    if (type === "jobSearch") {
      try {
        ...
        const { data } = await supabase
          .from("profiles")
          .select("job_searches_this_month, job_searches_reset_month")
          .eq("user_id", user.id)
          .single();
        const resetMonth =
          (data as any)?.job_searches_reset_month || "";
        ...
        await supabase
          .from("profiles")
          .update({
            job_searches_this_month: newCount,
            job_searches_reset_month: currentMonth,
          } as any)
          .eq("user_id", user.id);
        setSubscription((prev) => ({
          ...prev,
          jobSearchesThisMonth: newCount,
          jobSearchesResetMonth: currentMonth,
        }));
        return true;
      } catch (error) {
    ...
    try {
      ...
      const updateObj = { [field]: newCount } as Record<string, number>;
      await supabase
        .from("profiles")
      ...
      setSubscription((prev) => {
        const stateKey = {
          ...
          note: "notesToday",
        }[type as keyof typeof fieldMap] as keyof SubscriptionData;
